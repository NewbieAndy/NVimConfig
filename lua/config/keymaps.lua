...
map("n", "]]", "<C-i>", { desc = "forward", remap = true })
map("n", "[[", "<C-o>", { desc = "back", remap = true })
...
vim.api.nvim_create_user_command("SetProjectRoot", function()
	GlobalUtil.root.set_current_buffer_root()
end, { desc = "将当前 buffer 所属项目设为全局根目录" })
...
-- 是否打开markdown预览
map("n", "<leader>om", "<CMD>RenderMarkdown toggle<CR>", { desc = "切换 markdown 预览" })
