...
function M.status(icon, status)
  ...
  return {
    ...
    color = function()
      return GlobalUtil.ui.fg(colors[status()] or colors.ok)
    end,
  }
end
...
function M.cmp_source(name, icon)
  icon = icon or GlobalUtil.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
  local started = false
...
function M.pretty_path(opts)
  ...
  return function(self)
    ...
    local root = GlobalUtil.root.get({ normalize = true })
...
function M.root_dir(opts)
  opts = vim.tbl_extend("force", {
    ...
    icon = "󱉭 ",
    color = function()
      return GlobalUtil.ui.fg("Special")
    end,
  }, opts or {})
