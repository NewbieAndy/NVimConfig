...
return {
  ...
  opts = function()
    ...
    local has_words_before = GlobalUtil.cmp.has_words_before
    ...
    return {
      ...
      snippets = {
        preset = "default",
        expand = function(snippet) GlobalUtil.cmp.expand(snippet) end,
        active = function(filter) return vim.snippet.active(filter) end,
        jump = function(dir) vim.snippet.jump(dir) end,
      },
    ...
    if vim.g.blink_diag ~= false then
      ...
      vim.lsp.buf_request = function(buf, method, params, handler)
        ...
        local wrapped = function(err, result, ctx, cfg)
          ...
          if handler then return handler(err, result, ctx, cfg) end
